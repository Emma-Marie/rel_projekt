...
# Distance matrix
rs.dist <- dist(t(top_table)) # Euclidian distance
...
text(EuDistRS[, 1], EuDistRS[, 2], labels = colnames(top_table), cex = 0.8)# add column names as labels. "cex" adjusts the size of the text
...
plot(hc, labels = colnames(top_table)) # set column names (which is the CODE item names) as the words in the dendogram
...
# do the same but with ALL items (and not top 30)
#d.corr <- crossprod(d.bin)  # create a co-occurence matrix
#diag(d.corr) <- 0  # Make the diagonals 0s
## ploting
#network <- graph_from_adjacency_matrix(d.corr, mode = "undirected")
#plot(network)# Add labels for the top 30 items
