...
freq_plot <- function(dat){
  ...
  FREQ <- data.frame(SUM) # turn into data frame
  newdata <- FREQ[order(-FREQ$SUM),,drop = F] # sort the sums from most to least frequent
  barplot(newdata$SUM, names.arg = rownames(newdata), las = 2, ylab = "Frequency of words", main = paste0("Frequency analysis ", originalName), col = "darkseagreen")
...
# plot and save for religiosity
pdf(file = "fig_output/flowerplot_rel.pdf", width = 4, height = 4)
flo_plot(FL_REL, "rel")
dev.off()

# plot and save for spirituality
pdf(file = "fig_output/flowerplot_spir.pdf", width = 4, height = 4)
